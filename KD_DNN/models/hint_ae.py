...
class HintAE(object):
    ...
    def get_graph(self):
        ...
        with tf.variable_scope('loss'):
            ...
            ts_encoded = tf.nn.relu(tf.matmul(first_encoded, self.init_Y) + self.init_yBias)

            ts_loss = tf.square(self.encoded - ts_encoded)
...
def hintae(matrix_train, matrix_valid, iteration=100, lam=0.01, rank=50, rank2=50, seed=0, batch_size=256,
    ...
    model = HintAE(n, rank, rank2, lamb=lam, batch_size=batch_size, gpu_on=gpu_on, init_X=X, init_Y=Y, init_xBias=xBias,
                   init_yBias=yBias, confidence=confidence)
