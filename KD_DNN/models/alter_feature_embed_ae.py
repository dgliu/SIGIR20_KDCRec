...
class AlterFeatureEmbedAE(object):
    def __init__(self,
                 input_dim,
                 embed_dim,
                 batch_size,
    ...
    def get_graph(self):
        ...
        with tf.variable_scope('optimizer'):
            self.U_optimizer = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(self.loss,
                                                                                   var_list=[self.encode_weights,
                                                                                             self.encode_bias])
            self.V_optimizer = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(self.loss,
                                                                                   var_list=[self.decode_weights,
                                                                                             self.decode_bias])
...
def alterfeatureembedae(matrix_train, matrix_valid, iteration=100, lam=0.01, rank=50, seed=0, batch_size=256,
                        dataset=None, gpu_on=True, **unused):
    progress = WorkSplitter()

    progress.section("AlterFeatureEmbedAE: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("AlterFeatureEmbedAE: Load the variables trained on S_t")
    ...
    progress.section("AlterFeatureEmbedAE: Training")
    m, n = matrix_train.shape
    model = AlterFeatureEmbedAE(n, rank, lamb=lam, batch_size=batch_size, gpu_on=gpu_on, init_X=X, init_Y=Y,
