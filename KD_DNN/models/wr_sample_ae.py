...
class WRSampleAE(object):
    ...
    def get_graph(self):
        ...
        self.confidence = tf.get_variable(name='confidence', shape=[self.num_users, self.input_dim],
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = tf.reduce_mean(
                clip_confidence * self.mask * mf_loss + (1 - self.mask) * mf_loss) + self.lamb*tf.reduce_mean(l2_loss)
    ...
    def train_model(self, rating_matrix, marks, matrix_valid, epoch, metric_names):
        batches, marks_batches, sample_idx = self.get_batches(rating_matrix, marks, self.batch_size)
    ...
    def get_RQ(self, marks, rating_matrix):
        batches, _, _ = self.get_batches(rating_matrix, marks, self.batch_size)
        RQ = []
        for step in range(len(batches)):
