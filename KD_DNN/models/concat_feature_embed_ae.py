...
class ConcatFeatureEmbedAE(object):
    def __init__(self,
                 input_dim,
                 embed_dim,
                 batch_size,
    ...
    def get_graph(self):
        ...
        with tf.variable_scope('encode'):
            unif_encode_weights = tf.get_variable('unif_Weights', initializer=self.init_X, trainable=False)
            unif_encode_bias = tf.get_variable('unif_Bias', initializer=self.init_xBias, trainable=False)


            norm_encode_weights = tf.Variable(
                    tf.truncated_normal([self.input_dim, self.embed_dim], stddev=1 / 500.0),
                    name="norm_Weights")
            norm_encode_bias = tf.Variable(tf.constant(0., shape=[self.embed_dim]), name="norm_Bias")
        ...
        with tf.variable_scope('decode'):
            unif_decode_weights = tf.get_variable('unif_Weights', initializer=self.init_Y, trainable=False)
...
def concatfeatureembedae(matrix_train, matrix_valid, iteration=100, lam=0.01, rank=50, seed=0, batch_size=256,
                         dataset=None, gpu_on=True, **unused):
    progress = WorkSplitter()

    progress.section("ConcatFeatureEmbedAE: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("ConcatFeatureEmbedAE: Load the variables trained on S_t")
    ...
    progress.section("ConcatFeatureEmbedAE: Training")
    m, n = matrix_train.shape
    model = ConcatFeatureEmbedAE(n, rank, lamb=lam, batch_size=batch_size, gpu_on=gpu_on, init_X=X, init_Y=Y,
                                 init_xBias=xBias, init_yBias=yBias)
