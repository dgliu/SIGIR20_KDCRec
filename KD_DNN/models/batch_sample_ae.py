...
class BatchSampleAE(object):
    ...
    def train_model(self, rating_matrix, matrix_unif_train, matrix_valid, epoch, metric_names):
        ...
        unif_batches = self.get_batches(matrix_unif_train, self.batch_size)
        # Training
...
def batchsampleae(matrix_train, matrix_valid, matrix_unif_train, iteration=100, lam=0.01, rank=50, seed=0,
                  batch_size=256, gpu_on=True, step=3, **unused):
    ...
    model = BatchSampleAE(n, rank, lamb=lam, batch_size=batch_size, step=step, gpu_on=gpu_on)
    metric_names = ['NLL', 'AUC']

    RQ, X, xBias, Y, yBias = model.train_model(matrix_train, matrix_unif_train, matrix_valid, iteration, metric_names)
