...
class CausalSampleMF(object):
    ...
    def get_graph(self):
        ...
        self.item_embeddings = tf.get_variable(name='items', shape=[self.double_num_items, self.embed_dim],
                                               initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        self.user_bias_embeddings = tf.get_variable(name='users_bias', shape=[self.num_users, ],
                                                    initializer=tf.truncated_normal_initializer(stddev=0.01))
        self.item_bias_embeddings = tf.get_variable(name='items_bias', shape=[self.double_num_items, ],
                                                    initializer=tf.truncated_normal_initializer(stddev=0.01))
