...
class AlterFeatureEmbedMF(object):
    def __init__(self,
                 num_users,
                 num_items,
                 embed_dim,
    ...
    def get_graph(self):
        ...
        self.user_embeddings = tf.get_variable('users', initializer=self.init_U)
        self.item_embeddings = tf.get_variable('items', initializer=self.init_V)
        ...
        with tf.variable_scope("mf_loss"):
            ...
            items = tf.nn.embedding_lookup(self.item_embeddings, self.item_idx, name="items")
        ...
        with tf.variable_scope('optimizer'):
            self.U_optimizer = tf.contrib.opt.LazyAdamOptimizer().minimize(self.loss, var_list=[self.user_embeddings,
                                                                                                self.user_bias_embeddings])
            self.V_optimizer = tf.contrib.opt.LazyAdamOptimizer().minimize(self.loss, var_list=[self.item_embeddings,
        ...
        for i in tqdm(range(epoch)):
            for U_step in range(3):
                ...
                for step in range(len(batches)):
                    ...
                    _ = self.sess.run([self.U_optimizer], feed_dict=feed_dict)
            batches = self.get_batches(user_item_pairs, rating_matrix, self.batch_size)
            for step in range(len(batches)):
                feed_dict = {self.user_idx: batches[step][0],
                             self.item_idx: batches[step][1],
                ...
                _ = self.sess.run([self.V_optimizer], feed_dict=feed_dict)

            RQ, Y, uBias, iBias = self.sess.run([self.user_embeddings, self.item_embeddings,
...
def alterfeatureembedmf(matrix_train, matrix_valid, iteration=100, lam=0.01, rank=50, seed=0, batch_size=500,
                        gpu_on=True, dataset=None, **unused):
    progress = WorkSplitter()

    progress.section("AlterFeatureEmbedMF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("AlterFeatureEmbedMF: Load the variables trained on S_t")
    ...
    progress.section("AlterFeatureEmbedMF: Training")
    m, n = matrix_train.shape
    model = AlterFeatureEmbedMF(m, n, rank, lamb=lam, batch_size=batch_size, gpu_on=gpu_on, init_U=RQ, init_V=Y,
