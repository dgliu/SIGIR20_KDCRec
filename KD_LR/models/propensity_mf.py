...
class PropensityMF(object):
    ...
    def get_graph(self):
        ...
        with tf.variable_scope("mf_loss"):
            ...
            mf_loss = tf.reduce_mean(self.weight * tf.square(self.label - x_ij))
    ...
    @staticmethod
    def get_batches(user_item_pairs, rating_matrix, invP, batch_size):
        batches = []
        ...
        for i in range(int(len(user_item_pairs) / batch_size)):
            ...
            weight = invP[0] * np.ones(label.shape)
            weight[label == 1] = invP[1]

            batches.append([ui_pairs[:, 0], ui_pairs[:, 1], label, weight])
