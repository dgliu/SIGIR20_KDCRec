...
class UnionSampleMF(object):
    ...
    def get_graph(self):
        ...
        with tf.variable_scope("mf_loss"):
            ...
            mf_loss = self.confidence * self.mark * mf_loss + (1 - self.mark) * mf_loss
    ...
    def get_batches(user_item_pairs, marks, rating_matrix, batch_size):
        ...
        for i in range(int(len(user_item_pairs) / batch_size)):
            ...
            batches.append([ui_pairs[:, 0], ui_pairs[:, 1], label, mark])
...
def unionsamplemf(matrix_train, matrix_valid, matrix_unif_train, iteration=100, lam=0.01, rank=50, seed=0,
                  batch_size=500, confidence=0.9, gpu_on=True, **unused):
    ...
    model = UnionSampleMF(m, n, rank, lamb=lam, batch_size=batch_size, gpu_on=gpu_on, confidence=confidence)
    ...
    RQ, Y, user_bias, item_bias = model.train_model(matrix_train, marks, matrix_valid, iteration, metric_names)
